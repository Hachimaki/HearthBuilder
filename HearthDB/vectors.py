...
card_map = json.load(open('_vectors/card_map.json'))

class Deck:
    def __init__(self, arch_id: int, cards: list, win_rate: int):
...
def get_archs() -> list:
    ...
    """
    with open('_vectors/decks_by_archetype.json') as deckfile:
...
def get_decks(arch_id: int=None) -> list:
    ...
    with open('_vectors/decks_by_archetype.json') as deckfile:
        ...
        return [Deck(arch_id, d['cards'], d['win_rate']) for d in decks]
...
if __name__ == '__main__':
    # Read cards dataset
    with open('dataset.json') as dataset, open('_vectors/most_popular_cards.json') as most_common:
        ...
        with open('_vectors/decks_by_archetype.json', 'w') as outfile:
